    ...
    if ((flags & ImGuiColorEditFlags_DisplayHex) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        ImGui::BeginGroup();
        {
            ...
            if (picker)
            {
                ...
                ImColor color = ImColor(0, (int)ImGui::GetCursorPos().x, (int)ImGui::GetCursorPos().y);
                //ReleaseDC(NULL, dc);

                ImVec2 picker_rect_start = ImGui::GetMousePos() - ImVec2(20, 20);

                GetForegroundDrawList()->AddCircleFilled(picker_rect_start, 15.f, ImColor((float)(color.Value.x) / 255.f, (float)(color.Value.y) / 255.f, (float)(color.Value.z) / 255.f, 1.f));
                ...
                if (IsMouseReleased(ImGuiMouseButton_Left) && !IsItemHovered()) {
                    col[0] = (float)(color.Value.x) / 255.f;
                    col[1] = (float)(color.Value.y) / 255.f;
                    col[2] = (float)(color.Value.z) / 255.f;
                    col[3] = 255.f;
